...
def count_flops_and_params(cfg, model):
    ...
    if SIZE == [320, 320]:
        ...
        input_shape = (BATCH_SIZE, 3, 256, 128)
    flops, macs, params = get_model_profile(model=model, input_shape=input_shape)
    print(f"FLOPS: {flops}")
    print(f"MACS: {macs}")
...
def run_single(cfg, method, logger_save_dir):
    ...
    experiment_name = cfg.EXPERIMENT_NAME if cfg.EXPERIMENT_NAME != "" else "exp1"
    ...
    filename = f"profiles/{cfg.MODEL.NAME}_{cfg.INPUT.SIZE_TRAIN}_{'WITH LGT' if cfg.INPUT.USE_LGT else 'NO LGT'}_{'CE' if cfg.MODEL.USE_CENTROIDS else 'NO CE'}"
    filename += f"_{'WITH LGPR' if cfg.INPUT.USE_LGPR else 'NO LGPR'}_{'WITH GGPR' if cfg.INPUT.USE_GGPR else 'NO GGPR'}_{'WITH FUSE_RGB' if cfg.INPUT.USE_FUSE_RGB else 'NO FUSE_RGB'}_{'WITH RE-RANKING' if cfg.MODEL.USE_RE_RANKING else 'NO RE-RANKING'}"
    print(f"Skenario: {filename}")
    profiler = AdvancedProfiler(output_filename=filename)
    ...
    if cfg.TEST.ONLY_TEST:
        ...
        method.hparams.MODEL.USE_CENTROIDS = method.hparams.MODEL.USE_CENTROIDS
        # trainer.test(model=method, test_dataloaders=val_dataloader)
        ...
        if cfg.MODEL.RESUME_TRAINING:
            ...
            # count_flops_and_params(cfg, method)
        else:
            method = method(
                cfg,
