...
class ReidBaseDataModule(pl.LightningDataModule):
    ...
    def _print_dataset_statistics(self, train, val=None, query=None, gallery=None):
        num_train_pids, num_train_imgs, num_train_cams = self._get_imagedata_info(train)
        num_val_pids, num_val_imgs, num_val_cams = self._get_imagedata_info(val)
        ...
        print(
            "  val     | {:5d} | {:8d} | {:9d}".format(
                num_val_pids, num_val_imgs, num_val_cams
            )
        )
        print(
...
class COCODatasetBase(ReidBaseDataModule):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.json_val_path = self.json_train_path.replace("train", "val")
        self.json_test_path = self.json_train_path.replace("train", "test")
    ...
    def setup(self):
        ...
        self.val_json = self._load_json(self.json_val_path)
        # self.test_json = self._load_json(self.json_test_path)
        ...
        val, val_dict = self._process_dir(self.val_dir, self.val_json)
        # test, test_dict = self._process_dir(self.test_dir, self.test_json)
        ...
        self._print_dataset_statistics(train, val, query, gallery)
    ...
    def __getitem__(self, pid):
        ...
        if needPad:
            ...
            if self.resample:
                ...
                # img_mock = torch.zeros((3, 256, 128))
